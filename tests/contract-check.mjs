...
function checkAvatarAppContract(app) {
  ...
  const avatarChecks = [
    ...
    ["resets avatar timeline state after video seek", /video\?\.\s*addEventListener\(\s*["']seeked["'][\s\S]*?resetVideoTimelineState\(\s*["']seeked["']\s*\)/],
    ["detects video timeline rewind before duplicate frame skip", /const\s+videoTime\s*=\s*Number\(video\.currentTime[\s\S]*?shouldResetVideoTimeline\(videoTime\)[\s\S]*?resetVideoTimelineState\(\s*["']rewind["']\s*\)[\s\S]*?videoTime\s*===\s*state\.lastVideoTime/],
...
function checkAvatarRendererContract(avatarRenderer) {
  const rendererChecks = [
    ...
    ["resetPose resets both solver states and update clock", /function\s+resetPose\s*\(\s*options\s*=\s*\{\}\s*\)[\s\S]*?resetPoseSolverState\(poseSolverState\)[\s\S]*?resetPoseSolverState\(strictPoseSolverState\)[\s\S]*?lastUpdateTime\s*=\s*0/],
    ...
    ["keeps strict non-profile body bones free of rest-pose angle caps", /function\s+resolveBodyRetargetLimits\s*\([^)]*\)[\s\S]*strictModeActive\s*&&\s*!profile[\s\S]*maxAngle\s*:\s*undefined[\s\S]*maxTwist\s*:\s*undefined/],
