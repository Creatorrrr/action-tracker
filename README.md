...
The adapter can also convert generic external joint-array exports before validation. Use `--joint-format mediapipe33` for 33-joint MediaPipe-compatible arrays, or `--joint-format coco17` for common COCO-17 body joints. The input shape may provide `frames[].joints3d` / `frames[].worldJoints` and optional `frames[].joints2d` / `frames[].imageJoints`; the adapter maps them into replayable MediaPipe-33 landmarks and records the mapping in `sourceMeta`.
