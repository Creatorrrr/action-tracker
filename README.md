...
- Motion agreement: `motionAgreement` is the cross-model pass/fail score for uploaded humanoid rigs. It targets `95%` overall and at least `90%` for each gated component. The gated components are 3D bone-direction match and torso front/back side-order; neck/head direction and projected-segment direction stay in the report as viewport/rig-axis diagnostics so different character proportions or VRM rest axes are not treated as motion failures.
- Face/head pose: `faceHeadPose` reports MediaPipe face-transform layout diagnostics, face yaw/pitch/roll, final Head bone yaw/pitch/roll, reacquire/blend state, angular velocity, and jump counters. Use this when camera face direction and the avatar's visible head direction disagree.
...
- App pipeline performance: `window.motionTrackerDebug.getAppPerformanceReport()` exposes the active frame pump (`auto`, `rvfc`, or `raf`), callback/detection FPS, duplicate-frame counts, latest-wins frame count, stale rVFC callback drops, frame-age p95, frame-callback-lag p95, detect/process/draw/frame-total p95 timings, face detect/process p95 timings when face tracking is active, validation sampling status, MediaPipe delegate status, and tracking-worker status. Use `?pump=rvfc`, `?pump=raf`, or `?pump=auto` to compare schedulers. The app requests the MediaPipe `GPU` delegate for pose/hand/face detectors by default and falls back to `CPU` per detector while reporting the requested delegate, active detector delegates, detector-specific `attempted` delegate order, and `fallbackReasons`. Use `?delegate=cpu` for software/headless validation runs where SwiftShader makes GPU delegate timings misleading. `?tracking-worker=on` moves pose/hand/face detection into a module Web Worker when the browser supports `Worker`, `createImageBitmap`, and `OffscreenCanvas`; the worker loads MediaPipe's module wasm fileset and converts transferred video frames through `ImageData` before detection and reports the same delegate telemetry shape. If initialization or frame transfer fails, the app records `fallbackReason` and continues on the main-thread detector. Worker mode remains opt-in unless its p95 timings beat the default path. `?debug-overlay=off` disables the 2D landmark overlay for measurement runs without changing avatar retargeting. Use `window.motionTrackerDebug.getTrackedChannelReport()` to inspect the latest body, hand, face, expression, and finger-chain channel coverage.
...
- Retarget smoothing is enabled by default for live preview so landmark jitter does not drive every tiny avatar motion. Use `?smoothing=off` for raw/immediate comparison runs or `?smoothing=strong` for heavier smoothing, and inspect `window.motionTrackerDebug.getAvatarPerformanceReport().retargetSmoothing` or the `avatarSmoothingMode` field in `scripts/avatar-motion-agreement-check.mjs` reports.
- Avatar retargeting defaults to `strict`, which follows source skeleton joint directions directly for visible body/hand targets. Use `?avatar-retarget=legacy` to compare against the older smoothing/hold-heavy path, or `window.motionTrackerDebug.setAvatarRetargetMode("legacy")` at runtime.

The avatar retargeter keeps the default depth scale conservative at `0.45`, then applies dynamic segment-length calibration when enough world landmark samples are available. The HUD reports whether calibration is warming, ready, locked, static, or needs more body coverage. In strict retarget mode, visible body targets follow the source joint direction directly; missing targets and lost tracking still hold briefly, ease back toward rest pose, then blend live retarget strength back in after reacquisition. Legacy retarget mode keeps the older low-confidence hold and limb-plane secondary-axis behavior for comparison. Set `?depth-scale=1` only when inspecting depth residuals against the full MediaPipe depth signal; set `?depth-scale=0` for a flat baseline, which disables dynamic calibration.
...
For local video files, the app resets avatar temporal state when playback seeks or loops back to an earlier `currentTime`. This prevents root yaw and facing estimation from carrying the previous loop's final orientation into the first frame of the next loop.
...
- Disable it with `?face-tracking=off`, or call `await window.motionTrackerDebug.setFaceTrackingEnabled(false)`.
- Re-enable it with `?face-tracking=on`, or call `await window.motionTrackerDebug.setFaceTrackingEnabled(true)`.
- Use the **Face expressions** toggle in the Tracking panel for the same runtime control.
...
The avatar status row reports live face tracking and expression coverage. `Face` separates detector state such as `Tracked 52`, `Ready`, `No face`, and `Off`; `Expressions` reports whether the current avatar has usable blink, mouth, emotion, or look morph targets. This makes the common default-avatar case explicit: the face can be tracked even when the avatar has no expression targets to drive.

When enabled, the app loads MediaPipe `FaceLandmarker` from the same Tasks Vision runtime and requests blendshape scores plus a facial transformation matrix. The renderer maps MediaPipe/ARKit blendshape names to VRM0/VRM1 preset expressions such as blink, mouth visemes, smile, and look direction by driving GLTF morph targets directly. Tiny blendshape noise is suppressed with per-preset deadbands, visible expressions get conservative gain, and blink, mouth, emotion, and look presets use separate smoothing speeds that intentionally favor stable expressions over micro-movements. The facial transformation matrix is decoded with row/column-major layout diagnostics, normalized against a stable face base quaternion, held through short tracking gaps, and blended back in after reacquisition before it becomes a bounded Head/Neck pose correction. Full face landmarks are preserved only when explicitly requested. The default GLB path remains browser-first and dependency-light; VRM uploads use a pinned `@pixiv/three-vrm` loader path for VRM0 material compatibility, MToon rendering, and motion-gated spring-bone updates when the model contains spring metadata. The app still does not add Electron, MediaPipe Holistic, Kalidokit, FBX loading, or a full VTuber studio product surface.
...
The VRM check validates the VRM extension marker, VRM0/VRM1 humanoid metadata, required body mappings, three-segment finger chains, vertex-color alpha diagnostics, and VRM0 secondary animation metadata. Runtime `window.motionTrackerDebug.getAvatarRigReport()` also reports VRM expression coverage, rest-pose cache coverage, inferred bone orientation axes, unresolved node mappings, render compatibility sanitization, and current finger chain coverage. Head-axis inference prefers the Head bone's own parent-to-head local direction before child nodes so VRM eye, hair, or accessory nodes do not become the retarget axis. `window.motionTrackerDebug.getVrmRuntimeReport()` reports whether the optional VRM runtime is available, the disabled three-vrm humanoid auto-update state, spring-bone enablement, spring motion score/activity, active physics state, idle reset count, joint/collider counts, the last runtime update delta, and runtime update errors. When the avatar is still, `springPhysicsActive` should settle to `false`; body motion briefly reactivates spring updates while hair or clothing catches up. It reports the legacy `12 MB` / `60k` vertex / `110` bone budget as warnings only unless `--enforce-budget` is passed. Runtime budgets still use the in-browser avatar performance report; both the default Xbot and uploaded VRM should stay below update p95 `3ms`, render p95 `14ms`, and validation p95 `2ms` on the sample video.
...
Run the browser head-pose smoke with:

```bash
npm run smoke:head
```

It reuses the motion-agreement browser runner, writes `output/reports/head-pose-smoke-latest.json`, and checks `faceHeadPose` jump counters plus Head rest-axis diagnostics. The bundled dance sample does not always produce a MediaPipe face transform in headless Chrome, so face/bone yaw sign and correlation are diagnostic-only unless enough face yaw samples are present; the deterministic matrix-layout, mirror-sign, and drop/reacquire checks live in `tests/face-head-pose-check.mjs` and run as part of `npm run check`.

Run the root-yaw recovery smoke for the known 9-11s turn clip with:

```bash
npm run smoke:root-yaw
```

It reuses the motion-agreement browser runner on `output/test-videos/shorts-new-dance-E9_h_ZW5z0U-16x9-padded.mp4`, records the measured `motionFrame` timeline, and writes `output/reports/root-yaw-recovery-latest.json`. The smoke checks avatar root target error plus the 8.5-11.5s solver window. `rootYaw.reliabilityReasons`, `rootYaw.unreliableCount`, `rootYaw.recoveringCount`, and `solverWindow.stableAfterUnreliableCount` show whether transient side-order/raw-yaw instability was held and then reacquired. The same live fields are exposed through the pose solver snapshot as `facingYawReliable`, `facingYawReliabilityReason`, `facingRecoveringFromUnreliableYaw`, and `facingRecoveryTargetYawDeg`.

Anatomical retarget constraints are enabled by default. They clamp or confidence-scale implausible elbows, knees, shoulder/hip swing, lower-body reliability, and strict-mode hand/finger rotations before the avatar consumes the solver targets. Use `?anatomy-constraints=off` only for A/B debugging. Browser motion reports expose `poseSolver.anatomy.*`, and saved recordings can be summarized with:

```bash
node scripts/anatomy-constraint-smoke.mjs output/reports/arms-crossed-recording-xbot-anatomy-diagnostics.jsonl
```

See `docs/anatomical-constraints.md` for the constraint families, diagnostics, validation commands, and references.
...
- Avatar motion is an approximate retarget from MediaPipe pose and hand landmarks to supported humanoid bones, including Mixamo-style `mixamorig:*` names, common unprefixed humanoid names, and VRM0/VRM1 humanoid metadata. Limb bones use primary-direction swing retargeting so Mixamo-compatible uploads like Soldier can move the whole arm/leg instead of only the hands/feet. Pose-only head aim blends the nose offset into the virtual head-crown target so head yaw and pitch still move when a full face transform is unavailable, and the validation report now includes a separate Head direction diagnostic. Bounded head/neck/body/hand retarget smoothing is enabled by default; use `?smoothing=off` for immediate retarget application or `?smoothing=strong` when the source clip needs heavier jitter suppression. VRM models use a conservative anime profile that reduces head/neck strength, adds deadband and hysteresis, and applies bounded screen-space proportion calibration so short rigs do not overreact. It is intended as a local visual preview, not a production motion-capture solver.
- SysMocap is useful prior art for timeline recording, forwarding, and model workflow ideas, but this project intentionally does not clone SysMocap's Electron shell, OBS/WebXR product surface, MediaPipe Holistic runtime, Kalidokit dependency, FBX loader, or full VTuber studio surface. The default GLB path stays lightweight; uploaded VRM files use the pinned `@pixiv/three-vrm` runtime only for VRM material compatibility, MToon support, geometry/skeleton optimization, and motion-gated spring-bone updates.

## Runtime Notes

- The implementation loads MediaPipe Tasks Vision JavaScript and WASM assets from `cdn.jsdelivr.net`.
- Pose, hand, and face model files are loaded from the public MediaPipe model URLs on `storage.googleapis.com`.
...
- Face tracking and VRM expression mapping are enabled by default, but visible expression quality depends on the uploaded model's VRM expression or morph-target coverage. Full face-landmark recording remains opt-in. Iris-level lookAt, direct eye-bone rotation, manual binding UI, and FBX support are still deferred.
